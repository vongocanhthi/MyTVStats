/** Đồng bộ với `DEFAULT_DEEPSEEK_PROMPT` trong Rust (`src-tauri/src/models.rs`). */
export const DEFAULT_DEEPSEEK_PROMPT = `Bạn là chuyên viên phân tích phản hồi khách hàng của ứng dụng MyTV.
...
export const DEFAULT_DEEPSEEK_EMAIL_SUBJECT = "Báo cáo đánh giá MyTV";

/** Mẫu mail mặc định khi ngày báo cáo có 0 review — không gọi DeepSeek. */
export function buildZeroReviewReportBody(dayKey: string): string {
  return `Kính gửi Anh/Chị,

Dưới đây là tóm tắt đánh giá của khách hàng trên Google Play trong kỳ báo cáo ngày ${dayKey}.
